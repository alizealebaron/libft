...
/*   Updated: 2025/10/25 18:49:27 by alebaron         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	 ft_mod(void *content);
void	*ft_mod2(void *content);
...
int	main()
{
	...
	TestFonction tests_bonus[] = 
	{
		{"ft_lstnew_bonus"      , test_lstnew      , 0},
		{"ft_lstadd_front_bonus", test_lstadd_front, 0},
		{"ft_lstsize_bonus"     , test_lstsize     , 0},
		{"ft_lstlast_bonus"     , test_lstlast     , 0},
		{"ft_lstadd_back_bonus" , test_lstadd_back , 0},
		{"ft_lstdelone_bonus"   , test_delone      , 0},
		{"ft_lstclear_bonus"    , test_lstclear    , 0},
		{"ft_lstiter_bonus"     , test_lstiter     , 0},
		{"ft_lstmap_bonus"      , test_lstmap      , 0},
	};
	...
	int nombreBonus = sizeof(tests_bonus) / sizeof(tests_bonus[0]);


	for (int i = 0; i < nombreBonus; i++) 
	{
		afficherSeparateur(tests_bonus[i].nom);
		tests_bonus[i].etat = tests_bonus[i].test();
		printf("\n");
	}
	...
	for (int i = 0; i < nombreBonus; i++) 
	{
		if (tests_bonus[i].etat == 0)
			printf("%-25s %20s\n", tests_bonus[i].nom, "\033[31mKO\033[00m");
		else if (tests_bonus[i].etat == 1)
			printf("%-25s %20s\n", tests_bonus[i].nom, "\033[32mOK\033[00m");
		else
			printf("%-25s %20s\n", tests_bonus[i].nom, "\033[33m??\033[00m");
	}
...
int test_lstclear ()
{
    ...
    ft_lstclear(&t3, ft_del);
	
	if (t3 != NULL && t4 != NULL)
	{
		result = 0;
        printf("\033[31mKO\033[00m : t3 ou t4 n'a pas été nettoyé.\n");
	}
	else
	{
        printf("\033[32mOK\033[00m : Tout est bon.\n");
	}
...
int test_lstmap ()
{
	...
	char *s3 = strdup("Cua !"  );
	char *s4 = strdup("Pnak !" );
	char *lst_char[] = {"COIN !", "QUACK !", "CUA !", "PNAK !"};
    ...
    ft_lstiter(t1, ft_mod);

	debut = ft_lstmap(t1, ft_mod2, ft_del);
	
	int i = 0;
	while (debut)
	{
		printf("%s\n", (char *) (debut->content));
		if (strcmp((char *) debut->content,lst_char[i])) {result = 0;}
		debut = debut->next;
		i++;
	}
...
int test_lstiter ()
{
	...
	debut = NULL;
	ft_lstadd_front(&debut, t4);
	ft_lstadd_front(&debut, t3);
	ft_lstadd_front(&debut, t2);
	ft_lstadd_front(&debut, t1);
...
int test_delone ()
{
	...
	ft_lstdelone(t3, &ft_del);

    if (result) {printf("\033[32mOK\033[00m : Tout est bon !\n");}
...
int test_lstadd_back ()
{
	...
	debut = NULL;
	ft_lstadd_back(&debut, t1);
	ft_lstadd_back(&debut, t2);
	ft_lstadd_back(&debut, t3);
	ft_lstadd_back(&debut, t4);

	if (ft_lstlast(debut) != t4)
	{
		result = 0;
        printf("\033[31mKO\033[00m : La structure renvoyée n'est pas la bonne.\n");
	}
	else
	{
        printf("\033[32mOK\033[00m : La structure renvoyée est la bonne.\n");
	}
...
int test_lstsize ()
{
	...
	if (ft_lstsize(debut) != 4)
	{
		result = 0;
        printf("\033[31mKO\033[00m : La taille renvoyée n'est pas bonne.\n");
	}
	else
	{
        printf("\033[32mOK\033[00m : La bonne taille est renvoyée.\n");
	}
...
int test_lstadd_front ()
{
	...
	{
		printf("%s\n", (char *) (debut->content));
		if (debut->content != lst_char[i]) {result = 0;}
		debut = debut->next;
		i++;
	}
...
int test_lstnew ()
{
	...
	if (strcmp(t->content, str) != 0) {result = 0;}
	if (t->next != NULL             ) {result = 0;}
